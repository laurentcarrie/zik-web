...
export default function EditTexPage() {
  ...
  const { data: texData, isLoading: texLoading } = useQuery({
    queryKey: ['tex', id, filename],
    queryFn: () => fetchTex(song!.key, filename!),
    enabled: !!song?.key && !!filename,
  })

  const saveMutation = useMutation({
    mutationFn: () => saveTex(song!.key, filename!, content),
    onSuccess: () => {
